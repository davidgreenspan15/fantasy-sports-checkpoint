import { ScrapedLeague, ScrapedTeam } from "@prisma/client";
...
) => Promise<ScrapedTeam[]> = async (leagues) => {
  const teams: ScrapedTeam[] = [];
  const response = await Promise.all(
    leagues.map(async (l) => {
      await scrapeTeamList(l);
      return l;
    })
  );
...
const getTeamCityAndName: (team: ScrapedTeam) => Promise<ScrapedTeam> = async (
  team
) => {
  const { JSDOM } = jsdom;
